package sszt.character.loaders
{
	...
	public class MountsRunCharacterLoader extends LayerCharacterLoader 
	{
		
		public function MountsRunCharacterLoader(info:ICharacterInfo)
		{
		...
		override protected function initList(list:Array, idList:Array, needSexList:Array):void
		{
			...
			if (_info.style[3] != -1 && _info.style[3] != 0){
				...
				list.push(CharacterManager.pathManager.getSceneItemsPath(tmpPath, LayerType.SCENE_PLAYER +("_"+info.getCareer()) + (info.getSex() == 1 ? "_1" : "_2") +((_info.style[0] == 0 || clothT) ? "":"_"+_info.style[0])  + ( strength >= 10 ? "_1":"")));
			...
			if (_info.style[2] != -1 && _info.style[2] != 0)
			{
				...
				if(strength < 15)
				{
					tmpLayerType =  LayerType.SCENE_MOUNTS;
				}
				else
				{
					tmpLayerType = LayerType.SCENE_MOUNTS + "_" + Math.floor(strength  / 15);
				}
