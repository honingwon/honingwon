package sszt.scene.components.ActivityIcon
{
	 ...
	 * 
	 */	
	public class ClubActivityRaid extends BaseIconView
	{
		 ...
		 * 活动即将开启 倒计时 秒
		 */
		private var _actvieWillOpenTime:int = 1200;
		private var _activeData:ActiveStarTimeData;
		/**
		 * 活动id 
		 */
		private var _pvpActiveId:int = 1006;
		...
		public function ClubActivityRaid()
		{
			super();
//			_index = 13;
			init();
		...
		public static function getInstance():IActivityView
		{
			if(instance == null)
			{
				instance = new ClubActivityRaid() as IActivityView;
			}
			return instance;
		}
...
//			}
			...
			if(state == 1)
			{
				updateClubActiveStartTimeToOne();
			}
//			else if(state == 2)
//			{
...
//			}
		...
		private function init():void
		{
			...
			_btn = new MAssetButton1(AssetUtil.getAsset("ssztui.scene.TopBtnClubTXAsset") as MovieClip);
		...
		override protected function initEvent():void
		{
			...
			_btn.addEventListener(MouseEvent.MOUSE_OVER,overHandler);
			_btn.addEventListener(MouseEvent.MOUSE_OUT,outHandler);
			_countDown.addEventListener(Event.COMPLETE,completeClubAct);
			ModuleEventDispatcher.addModuleEventListener(ActiveStartEvents.ACTIVE_START_UPDATE,updateClubActiveTime);
		}
		 ...
		 * 活动开启
		 */		
		private function updateClubActiveStartTimeToOne():void
		{
			_countDown.visible = true;
		...
		private function completeClubAct(evt:Event):void
		{
			_effect.visible = true;
			_countDown.visible = false;
		}
		
		private function updateClubActiveTime(evt:ModuleEvent):void
		{
		...
		private function onClick(e:MouseEvent):void
		{
			...
			initData();
			//断玩家等级是否达到
			if(GlobalData.selfPlayer.level < 30 || GlobalData.selfPlayer.clubId == 0)
			{
				SetModuleUtils.addActivity(new ToActivityData(2,0,23));
			}
			else
			{
		...
		{
			TipsUtil.getInstance().show(LanguageManager.getWord("ssztl.activity.ActivityRaid"),null,new Rectangle(e.stageX,e.stageY,0,0));
		}
		...
		override protected function removeEvent():void
		{
			...
			_countDown.removeEventListener(Event.COMPLETE,completeClubAct);
			ModuleEventDispatcher.removeModuleEventListener(ActiveStartEvents.ACTIVE_START_UPDATE,updateClubActiveTime);
		}
