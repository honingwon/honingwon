package sszt.scene.components.shenMoWar.myWar
{
	...
	public class ShenMoAttackInfoTabPanel extends Sprite implements IShenMoMyWarInfoInterface
	{
		...
		public function ShenMoAttackInfoTabPanel(argMediator:SceneWarMediator)
		{
			super();
			_mediator = argMediator;
			initialView();
			_mediator.shenMoWarInfo.initShenMoWarMyWarInfo();
			initialEvents();
			_mediator.sendMyWarList();
		}
		...
		{
			_mediator.shenMoWarInfo.shenMoWarMyWarInfo.addEventListener(SceneShenMoWarUpdateEvent.SHENMO_MYWAR_INFO_UPDATE,initData);
		}
		
		private function removeEvents():void
		{
			_mediator.shenMoWarInfo.shenMoWarMyWarInfo.removeEventListener(SceneShenMoWarUpdateEvent.SHENMO_MYWAR_INFO_UPDATE,initData);

		}
		
		private function initData(e:SceneShenMoWarUpdateEvent):void
		{
			clearList();
			for each(var i:ShenMoWarMyWarItemInfo in myWarInfo.attackInfoList)
			{
				var tmpItemView:ShenMoWarMyWarItemView = new ShenMoWarMyWarItemView(_mediator,i);
		...
		public function get myWarInfo():ShenMoWarMyWarInfo
		{
			return _mediator.shenMoWarInfo.shenMoWarMyWarInfo;
		}
		...
		{
			removeEvents();
			_mediator.shenMoWarInfo.clearShenMoWarMyWarInfo();
			if(_bg)
			{
				_bg.dispose();
				_bg = null;
			...
			for each(var i:ShenMoWarMyWarItemView in _itemList)
			{
				if(i)
				{
					i.dispose();
