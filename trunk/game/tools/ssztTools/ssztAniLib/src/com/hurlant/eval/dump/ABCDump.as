...
package com.hurlant.eval.dump
{
	import flash.utils.ByteArray;
	
	public class ABCDump {

		static public function dump(data:ByteArray):String { 
			printBuffer = [];
			data.position = 0;
...
var metadata:Array;	
	...
	class Abc
	{
		public function verbose(start:uint, comment:String):void {
		...
		function getU32size(pos:uint):int {
			var v:int = data[pos];
			if (!(v & 0x00000080))
				return 1;
			v = v & 0x0000007f | data[pos+1]<<7;
			if (!(v& 0x00004000))
				return 2;
			v = v & 0x00003fff | data[pos+2]<<14;
			if (!(v & 0x00200000))
				return 3;
			v = v & 0x001fffff | data[pos+3]<<21;
			if (!(v & 0x10000000))
				return 4;
			return 5;
		}
		...
		function parseCpool()
		{
			...
			// ints
			n = readU32();
			verbose(p, "Pool of "+(n-1)+" int");
			ints = [0]
			for (i=1; i < n; i++) {
				p = data.position;
				ints[i] = readU32()
				verbose(p, "int["+i+"] = "+ints[i]);
			}
				
			// uints
			p = data.position;
			n = readU32()
			verbose(p, "Pool of "+(n-1)+" uint");
			uints = [0]
			for (i=1; i < n; i++) {
				p = data.position;
				uints[i] = uint(readU32())
				verbose(p, "uint["+i+"] = "+uints[i]);
			}
				
			// doubles
			p = data.position;
			n = readU32()
			verbose(p, "Pool of "+(n-1)+" Number");
			doubles = [NaN]
			for (i=1; i < n; i++) {
				p = data.position;
				doubles[i] = data.readDouble()
				verbose(p, "double["+i+"] = "+doubles[i]);
			}
			...
			p = data.position;
			n = readU32()
			verbose(p, "Pool of "+(n-1)+" String");
			strings = [""]
			for (i=1; i < n; i++) {
				p = data.position;
				var l:int = readU32();
				strings[i] = data.readUTFBytes(l);
				verbose(p, "String["+i+"] = \""+strings[i]+'"');
			}
			...
			p = data.position;
			n = readU32()
			verbose(p, "Pool of "+(n-1)+" Namespace");
			namespaces = [publicNs]
			for (i=1; i < n; i++) {
				p = data.position;
				var t:int = data.readByte();
				switch (t)
				{
				...
				case CONSTANT_StaticProtectedNs2:
				{
					...
					verbose(p, "Namespace["+i+"] = "+s+" \""+namespaces[i]+'"');
				...
				case CONSTANT_PrivateNs:
					...
					verbose(p, "Namespace["+i+"] = \""+namespaces[i]+'"');
					break;
				}
			}
			...
			p = data.position;
			n = readU32()
			verbose(p, "Pool of "+(n-1)+" namespace sets");
			nssets = [null]
			for (i=1; i < n; i++) {
				p = data.position;
				var count:int = readU32();
				verbose(p, "nsset["+i+"] has "+count+" namespaces");
				var nsset = nssets[i] = []
				for (j=0; j < count; j++) {
					p = data.position;
					nsset[j] = namespaces[readU32()]
					verbose(p, "nsset["+i+"]["+j+"] = \""+nsset[j]+"\"");
				}
			}
			...
			p = data.position;
			n = readU32()
			verbose(p, "Pool of "+(n-1)+" multinames");
			names = [null]
			namespaces[0] = anyNs
			strings[0] = "*" // any name
			for (i=1; i < n; i++) {
		...
		function parseMethodInfos()
		{
			...
			verbose(p, "Pool of "+method_count+" methods");
			methods = []
		...
		function parseMetadataInfos()
		{
			var p:int = data.position;
			var count:int = readU32()
			verbose(p, "Pool of "+count+" metadata");
			metadata = []
		...
		function parseInstanceInfos()
		{
			...
			var p:int = data.position;
			var count:int = readU32()
			verbose(p, "Pool of "+count+" instance info");
			instances = []
			for (var i:int=0; i < count; i++) {
				p = data.position;
		...
		function parseScriptInfos()
		{
			...
			var p:int = data.position;
			var count:int = readU32();
			verbose(p, "Pool of "+count+" script");
			scripts = []
			for (var i:int=0; i < count; i++)
	        {
	        	p = data.position;
		...
		function parseMethodBodies()
		{
			...
			var p:int = data.position;
			var count:int = readU32()
			verbose(p, "Pool of "+count+" method body");
			for (var i:int=0; i < count; i++)
	        {
	        	p = data.position;
	        	var iter:int = readU32();
	        	var m = methods[iter]
	        	if( m.name == "frame704" )
	        	{
	        		var dummy:* = null;
...
//			}
	...
	class Swf
	{
		...
		private function decodeTags()
		{
	        ...
	        {
	        	print("reading tag type at pos="+data.position);
	            type = (h = data.readUnsignedShort()) >> 6;
	            print("type="+type+" (h="+h+")");
	
	            if (((length = h & 0x3F) == 0x3F)) {
		        	print("reading tag length at pos="+data.position);
	                length = data.readInt();
	            }
