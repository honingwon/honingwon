package sszt.rank.components.views.equip
{
	...
	import sszt.rank.components.itemView.equip.EquipRankItemView;
	...
	public class EquipRankView extends RankViewImpl
	{
		...
		private var _curItemView:EquipRankItemView;
		
		private var _colWidth:Array = [51,130,174,110];
		
		public function EquipRankView(rankInfos:RankInfo)
		{
		...
		private function initView():void
		{
			...
			addChild(MBackgroundLabel.getDisplayObject(new Rectangle(colX[0]+2,3,_colWidth[1],16),new MAssetLabel(LanguageManager.getWord("ssztl.common.equipName"),MAssetLabel.LABEL_TYPE_TITLE2)));
			...
			addChild(MBackgroundLabel.getDisplayObject(new Rectangle(colX[2]+2,3,_colWidth[3],16),new MAssetLabel(LanguageManager.getWord("ssztl.common.equipFight"),MAssetLabel.LABEL_TYPE_TITLE2)));
		...
		public function setData():void{
			...
			_resultViewList = [];
			var i:int;
			var itemView:EquipRankItemView;
			var flag:Boolean;
			for(i = 0; i < _itemInfoList.length; i++)
			{
				itemView = new EquipRankItemView(_itemInfoList[i]);
		...
		//				if(list && list[i])
		//				{
		//					itemView = new EquipRankItemView(list[i]);
		//					itemView.addEventListener(MouseEvent.CLICK,itemClickHandler);
		//					_resultViewList.push(itemView);
		//					_tile.appendItem(itemView);
		...
		{
			curItemChange(evt.currentTarget as EquipRankItemView);
		}
		
		private function curItemChange(item:EquipRankItemView):void
		{
		...
		override public function dispose():void
		{
			...
			if(_resultViewList)
			{
				for each(var itemView:EquipRankItemView in _resultViewList)
				{
					itemView.dispose();
					itemView = null;
				}
