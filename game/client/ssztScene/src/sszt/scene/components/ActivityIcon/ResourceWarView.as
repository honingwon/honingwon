package sszt.scene.components.ActivityIcon
{
	 ...
	 * 资源战
	 */	
	public class ResourceWarView extends BaseIconView
	{
		 ...
		 * 活动id 
		 */
		private var _activeId:int = 1008;
		...
		public function ResourceWarView()
		{
			super();
			init();
			initEvent();
		...
		public static function getInstance():IActivityView
		{
			if(instance == null)
			{
				instance = new ResourceWarView() as IActivityView;
			}
			return instance;
		}
		...
		{
			_btn = new MAssetButton1(AssetUtil.getAsset("ssztui.scene.TopBtnResourceWarAsset") as MovieClip);
			addChild(_btn);
		...
		private function onClick(e:MouseEvent):void
		{
			...
			{
				SetModuleUtils.addActivity(new ToActivityData(2,0,27));
			}
			else
			{
		...
		{
			TipsUtil.getInstance().show(LanguageManager.getWord("ssztl.scene.resourceWarTip"),null,new Rectangle(e.stageX,e.stageY,0,0));
		}
