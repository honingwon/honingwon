package sszt.scene.components.ActivityIcon
{
	...
	public class BigBossIconView extends BaseIconView
	{
		...
		public static function getInstance():IActivityView
		{
			if(instance == null)
			{
				instance = new BigBossIconView() as IActivityView;
			}
			return instance;
		}
		...
		private function init():void
		{
			...
			_btn = new MAssetButton1(AssetUtil.getAsset("ssztui.scene.TopBtnBigBossAsset") as MovieClip);
			addChild(_btn);
		...
		{
			TipsUtil.getInstance().show(LanguageManager.getWord("ssztl.scene.bigBossWarTip"),null,new Rectangle(e.stageX,e.stageY,0,0));
		}
