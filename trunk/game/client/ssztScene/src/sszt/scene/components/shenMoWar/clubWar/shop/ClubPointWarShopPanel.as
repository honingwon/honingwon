package sszt.scene.components.shenMoWar.clubWar.shop
{
	...
	public class ClubPointWarShopPanel extends MPanel
	{
...
//		private var _tabBtns:Vector.<MCacheTab1Btn>;
		...
		private var _detailPanel:ClubPointWarShopItemDetailView;
		private var _currentCell:ClubPointWarShopItemCell;
		private var _leftHonorField:MAssetLabel;
//		private var _leftCopperField:MAssetLabel;
		
		public function ClubPointWarShopPanel(mediator:SceneWarMediator)
		{
			_mediator = mediator;
			var title:Bitmap;
...
//			_items = new Vector.<ShopItemCell>(20);
			...
			{
				var item:ClubPointWarShopItemCell = new ClubPointWarShopItemCell();
				_tile.appendItem(item);
				_items[j] = item;
				item.addEventListener(MouseEvent.CLICK,itemClickHandler);
			}
			
			_detailPanel = new ClubPointWarShopItemDetailView();
...
//		}
		...
		{
			for each(var i:ClubPointWarShopItemCell in _items)
			{
				i.shopItemInfo = null;
			}
//			var list:Vector.<ShopItemInfo> = _shopTemplate.getItems(_pageSize,_pageView.currentPage - 1,_currentIndex);
		...
		{
			setSelected(evt.currentTarget as ClubPointWarShopItemCell);
		}
		
		private function setSelected(cell:ClubPointWarShopItemCell):void
		{
			if(!cell.info)return;
		...
		override public function dispose():void
		{
			...
			if(_items)
			{
				for each(var cell:ClubPointWarShopItemCell in _items)
				{
