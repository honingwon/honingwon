package sszt.scene.actions.characterActions
{
	...
	public class MonsterAttackAction extends CharacterAttackAction
	{
...
//		private var _attackEffect:BaseLoadEffectPool;
		...
		public function MonsterAttackAction(info:AttackActionInfo)
		{
			super(info);
			_life = 15;
		}
		...
		override public function doAction():void
		{
			...
			if(_character && _character.scene)
			{
				...
				if(!(_character.sceneX == action.targetX && _character.sceneY == action.targetY))
				{
					var dir:int = DirectType.checkDir(new Point(_character.sceneX,_character.sceneY),new Point(action.targetX,action.targetY));
		...
		override public function doAttack():void
		{
			if(_character && _character.scene)
			{
				...
				if(_skill)
				{
					...
					if(movieInfo)
					{
						if(movieInfo.isMove)
						{
							...
							else
							{
								...
								_character.scene.addEffect(_effectPackage);
					...
					if(!SharedObjectManager.hideSkillEffect.value)
					{
						...
						if(_skill.getActionEffect()[0] > 0)
						{
							...
							if(actionMovieInfo)
							{
								...
								_actionEffect.move(_character.sceneX,_character.sceneY);
								_character.scene.addEffect(_actionEffect);
					...
					if(_skill.isShake)
						(_character as BaseSceneMonster).shakeScene();
				}
			}
		}
		/**
			...
			if(!SharedObjectManager.hideSkillEffect.value)
			{
				if(movie)
				{
					if(movie.count == 1)
					{
						...
						_completeEffect.play(SourceClearType.TIME,300000,4);
						if(_character && _character.scene)
