...
function TableBodyRow(rowNum,data,msg){
	...
	var ophtm1 = '[<a href="javascript:void(0);" onclick="javascript:DoServerRight(\'' + data.bm_AccountID + '\',\'' + data.bm_AccountName + '\')">设置服务器权限</a>]'+
				'[<a href="javascript:void(0);" onclick="javascript:DoModuleRights(\'' + data.bm_AccountID + '\',\'' + data.bm_AccountName + '\')">设置功能权限</a>]'+
				'[<a href="javascript:void(0);" onclick="javascript:DoGroup(\'' + data.bm_AccountID + '\',\'' + data.bm_AccountName + '\')">设置所在分组</a>]';
	var ophtm = '[<a href="javascript:void(0);" onclick="javascript:Edit(\'' + data.bm_AccountID + '\')">修改信息</a>]'+
				'[<a href="javascript:void(0);" onclick="javascript:Del(\'' + data.bm_AccountID + '\')">删除</a>]';
	var html = "<tr class='"+className+"'><td>"+data.bm_Account+"</td><td>"+data.bm_AccountName+"</td><td class='tc_gray'>"+ophtm1+"</td><td class='operate'>"+ophtm+"</td></tr>";
   return  html;
}
...
function btnAccountSaveClickHandler()
{
	...
	var fEmail = checkEmail();
	if(fAccount && fName && fType && fEmail && checkPhone() && checkQQ() && checkAdress() && checkRemark()){
		...
		if(_AccountID == null){
			var dataStr = {"method": "Add","account":AccountID,"name":NickName,"phone":Phone,"mail":Email,"qq":QQ,"adr":Address,"type":AccountType,"dec":ARemark};
			$.ajax({type: "post",url: _URL,dataType: "json",data: dataStr,
				success: function(result) {
					if(result.Success){
						alert("新增成功");
			...
			var dataStr1 = {"method": "edit","name":NickName,"phone":Phone,"mail":Email,"qq":QQ,"adr":Address,"type":AccountType,"dec":ARemark,"ID":_AccountID};
...
{
	$("#"+_pageAccountID).val(data == null ? "" : data.bm_Account);
	$("#"+_pageNickName).val(data == null ? "" : data.bm_AccountName);
	document.getElementById(_pageSelectAccountType).value = data == null ? 2 : data.bm_AccountType;
	...
	$("#"+_pageARemark).val(data == null ? "" : data.bm_ARemark);
	setOnbur(_pageAccountID,"");
	setOnbur(_pageNickName,"");
	setOnbur(_pageSelectAccountType,"");
	setOnbur(_pagePhone,"");
