    ...
    if (ListData == null || ListData.IsSuccess == false) {
        ...
        if (DataList == null || DataList.length == 0) {
            ...
            while (row != null) {
            	...
            	var ophtm = '[<a href="javascript:void(0);" onclick="javascript:Update(\'' + row.group_id + '\',this)">修改</a>]'+
            				'[<a href="javascript:void(0);" onclick="javascript:DoGroupModuleRights(\'' + row.group_id + '\',\'' + row.group_name + '\')">设置功能权限</a>]';
            	var className = new Array();
            	className.push("");className.push("");className.push("operate");
                TbodyObj.AddRow(new Array(row.group_id, row.group_name, row.group_remark, ophtm),className,null,null,null,null,"even");
                i++;
                row = DataList[i];
                $("tr").addClass("even");  
            }
...
function Update(value,obj)
{
	...
	if(_List != null){
		for(var i = 0 ;i < _List.length ; ++i){
		   if(value == _List[i].group_id){
			   _CurrentInfo = _List[i];
			   break;
		   }
		}
		if(_CurrentInfo != null){
			initInputData(_CurrentInfo.group_name,_CurrentInfo.group_remark);
		}
	}
}
...
function btnSaveClick()
{
	...
	if(falgName && falgDesc){
		if(_CurrentInfo == null){
			...
			var dataStr = {"method": "edit","name": $("#name").val(),"dec":$("#dec").val(),"ID":_CurrentInfo.group_id};
			$.ajax({type: "post",url: _URL,dataType: "json",data: dataStr,
				success: function(result) {
					if(result.Success){
						alert("修改成功");
