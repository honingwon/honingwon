package sszt.rank.components.views.copy
{
	...
	import sszt.rank.components.itemView.copy.MultiClimbingTowerRankItemView;
	...
	public class MultiClimbingTowerRankView extends RankViewImpl
	{
		private var _bg:IMovieWrapper;
		private var _resultViewList:Array;
		private var _tile:MTile;
		private var _rankInfos:RankInfo;
		//		private var _curItemView:SingleCopyRankItemView;
		
		private var _colWidth:Array = [60, 240, 175];
		
		public function MultiClimbingTowerRankView(rankInfos:RankInfo)
		{
			super();
		...
		{
			var itemView:MultiClimbingTowerRankItemView;
			...
			{
				itemView = new MultiClimbingTowerRankItemView(list[i]);
		...
		{
			curItemChange(evt.currentTarget as MultiClimbingTowerRankItemView);
		}
		
		private function curItemChange(item:MultiClimbingTowerRankItemView):void
		{
			item.select = true;
		}
		...
		override public function dispose():void
		{
			...
			if(_resultViewList)
			{
				for each(var itemView:MultiClimbingTowerRankItemView in _resultViewList)
				{
					itemView.dispose();	
					itemView = null;
				}
