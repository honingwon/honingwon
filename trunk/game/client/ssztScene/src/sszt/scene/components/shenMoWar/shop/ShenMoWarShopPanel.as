package sszt.scene.components.shenMoWar.shop
{
	...
	import sszt.scene.components.shenMoWar.clubWar.shop.ClubPointWarShopItemCell;
	import sszt.scene.mediators.SceneWarMediator;
	
	public class ShenMoWarShopPanel extends MPanel
	{
...
//		private var _tabBtns:Vector.<MCacheTab1Btn>;
		...
		private var _detailPanel:ShenMoWarShopItemDetailView;
		private var _currentCell:ShenMoWarShopItemCell;
		private var _leftHonorField:MAssetLabel;
//		private var _leftCopperField:MAssetLabel;
		
		public function ShenMoWarShopPanel(mediator:SceneWarMediator)
		{
			_mediator = mediator;
			var title:Bitmap;
...
//			addContent(label1);
			...
			label3.move(292,39);
			addContent(label3);
			var label4:MAssetLabel = new MAssetLabel(LanguageManager.getWord("ssztl.common.buyItem"),MAssetLabel.LABELTYPE14);
			label4.move(292,140);
			addContent(label4);
...
//			_items = new Vector.<ShopItemCell>(20);
			...
			{
				var item:ShenMoWarShopItemCell = new ShenMoWarShopItemCell();
				_tile.appendItem(item);
				_items[j] = item;
				item.addEventListener(MouseEvent.CLICK,itemClickHandler);
			}
			
			_detailPanel = new ShenMoWarShopItemDetailView();
			_detailPanel.move(244,38);
			addContent(_detailPanel);
			
//			_cashBtn = new MCacheAsset1Btn(2,"兑换");
...
//		}
		...
		private function cashClickHandler(evt:MouseEvent):void
		{
			 ...
			 if(_currentCell.shopItemInfo.payType == 6)
			{
				...
				{
					QuickTips.show(LanguageManager.getWord("ssztl.scene.honorValueNoEnough"));
					return;
				}
			}
		...
		{
			for each(var i:ShenMoWarShopItemCell in _items)
			{
				i.shopItemInfo = null;
			}
//			var list:Vector.<ShopItemInfo> = _shopTemplate.getItems(_pageSize,_pageView.currentPage - 1,_currentIndex);
		...
		{
			setSelected(evt.currentTarget as ShenMoWarShopItemCell);
		}
		
		private function setSelected(cell:ShenMoWarShopItemCell):void
		{
			if(!cell.info)return;
		...
		override public function dispose():void
		{
			...
			if(_items)
			{
				for each(var cell:ShenMoWarShopItemCell in _items)
				{
