...
class RightsProvider
{
    ...
    public function GetModuleRightsByAccountID($Account)
    {
    	...
    	$sql.= " WHERE a.group_id = b.group_id AND a.account_id = ".$Account;
    	$sql.= " UNION";
    	$sql.= " SELECT module_id FROM bm_account_module WHERE account_id = ".$Account;
    	$r = sql_fetch_rows($sql);
    	$rights = Array();
    	if(count($r) > 0){
 			for ($i = 0 ; $i < count($r); $i++){
    ...
    public function UpdateModuleRight($AccountID,$addStr,$delStr)
    {
    	...
    	try{
    		$msg = "";
    		if(!empty($delStr)){
    			$sqldel ="delete from bm_account_module where account_id = ".$AccountID." AND module_id IN (".$delStr.")";
    			$r = sql_query($sqldel);
    			if($r == 0)
    		  	  $msg ="删除模块权限失败！";
    		}
    		if(!empty($addStr)){
    			$sqladd = "insert into bm_account_module (account_id,module_id) values ".$addStr;
    			$add = sql_query($sqladd);
    			if($add == 0)
    				$msg.="新增模块权限失败！";
    		}
    ...
    public function GetGroupInfoByAccount($AccountID)
    {
    	...
    	$sql = "SELECT group_id from bm_account_group where account_id = ".$AccountID;
    	$r = sql_fetch_rows($sql);
    	$array = Array();
    	if(count($r) > 0){
 			for ($i = 0 ; $i < count($r); $i++){
    ...
    public function UpdateAccountGroup($AccountID,$addStr,$delStr)
    {
    	...
    	try{
    		$msg = "";
    		if(!empty($delStr)){
    			$sqldel ="delete from bm_account_group where account_id = ".$AccountID." AND group_id IN (".$delStr.")";
    			$r = sql_query($sqldel);
    			if($r == 0)
    		  	  $msg ="删除账号所在分组失败！";
    		}
    		if(!empty($addStr)){
    			$sqladd = "insert into bm_account_group (account_id,group_id) values ".$addStr;
    			$add = sql_query($sqladd);
    			if($add == 0)
    				$msg.="新增账号所在分组失败！";
    		}
    ...
    public function GetGroupModuleRightsByGroup($GroupID)
    {
    	...
    	$sql = "SELECT module_id from bm_group_module where group_id = ".$GroupID;
    	$r = sql_fetch_rows($sql);
    	$array = Array();
    	if(count($r) > 0){
 			for ($i = 0 ; $i < count($r); $i++){
    ...
    public function UpdateGroupMoudleRights($GroupID,$addStr,$delStr)
    {
    	...
    	try{
    		$msg = "";
    		if(!empty($delStr)){
    			$sqldel ="delete from bm_group_module where group_id = ".$GroupID." AND module_id IN (".$delStr.")";
    			$r = sql_query($sqldel);
    			if($r == 0)
    		  	  $msg ="删除分组模块权限失败！";
    		}
    		if(!empty($addStr)){
    			$sqladd = "insert into bm_group_module (group_id,module_id) values ".$addStr;
    			$add = sql_query($sqladd);
    			if($add == 0)
    				$msg.="新增分组模块权限失败！";
    		}
