package sszt.activity.components
{
	...
	import sszt.activity.components.itemView.TaskItemView;
	 ...
	 * 每日任务
	 */
	public class TaskTabPanel extends Sprite implements IActivityTabPanel
	{
		...
		private var _currentItem:TaskItemView;
		
		private var _pageView:PageView;
		private var _currentPage:int = 1;
		
		public static var COLWidth:Array = [170,70,70,110];
		
		public function TaskTabPanel(argMediator:ActivityMediator)
		{
		...
		private function initView():void
		{
			...
			_bg = BackgroundUtils.setBackground([
				...
				new BackgroundInfo(BackgroundType.DISPLAY,new Rectangle(485,12,65,15),new Bitmap(new TaskAboutTagAsset() as BitmapData)),
				...
				new BackgroundInfo(BackgroundType.DISPLAY,new Rectangle(6,10,COLWidth[0],16),new MAssetLabel(LanguageManager.getWord("ssztl.common.name"),MAssetLabel.LABEL_TYPE_TITLE2)),
				...
				new BackgroundInfo(BackgroundType.DISPLAY,new Rectangle(6+colX[1],10,COLWidth[2],16),new MAssetLabel(LanguageManager.getWord("ssztl.common.progressLabel"),MAssetLabel.LABEL_TYPE_TITLE2)),
...
//			addChild(_pageView);
		...
		private function initTemplateData():void
		{
			...
			for each(var activityTaskTemplate:ActivityTaskTemplateInfo in ActivityTaskTemplateInfoList.list)
			{
				var task:TaskTemplateInfo = TaskTemplateList.getTaskTemplate(activityTaskTemplate.taskId);
				...
				if(selfLevel >= task.minLevel && selfLevel <= task.maxLevel && matchCareer)
				{
					var itemView:TaskItemView = new TaskItemView(activityTaskTemplate);
		...
		private function initEvent():void
		{
			...
			{
				TaskItemView(_itemList[i]).addEventListener(MouseEvent.CLICK,itemClickHandler);
			}
		}
		...
		private function removeEvent():void
		{
			...
			for(var i:int = 0; i < _itemList.length; i++)
			{
				TaskItemView(_itemList[i]).removeEventListener(MouseEvent.CLICK,itemClickHandler);
			}
		}
		
		private function switchTo(taskItemView:TaskItemView):void
		{
			if(_currentItem)
			{
				_currentItem.selected = false;
			}
			_currentItem = taskItemView;
		...
		{
			var target:TaskItemView = evt.currentTarget as TaskItemView;
		...
		private function updateView():void
		{
			...
			var cell:AwardCell;
			var awards:Array = _currentItem.taskInfo.awards;
			...
			_desciptionLabel.htmlText = _currentItem.taskInfo.description;
		}
