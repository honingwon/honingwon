package sszt.scene.components.shenMoWar.crystalWar
{
	...
	public class CrystalWarRelivePanel extends MPanel
	{
		...
		private var _reliveBtn4:MCacheAsset3Btn;
		...
		public function CrystalWarRelivePanel(argMediator:ReliveMediator)
		{
			...
			super(new MCacheTitle1("",title),true,0,false);
			initEvents();
		}
		...
		{
			super.configUI();
			setContentSize(227,188);
			
			_bg = BackgroundUtils.setBackground([
				new BackgroundInfo(BackgroundType.BORDER_2,new Rectangle(0,0,227,188)),
				new BackgroundInfo(BackgroundType.BORDER_6,new Rectangle(51,4,124,17))
			]);
...
//			_reliveYuanBaoBtn.addEventListener(MouseEvent.CLICK,btnClickHandler);
			...
			_reliveBtn4.addEventListener(MouseEvent.CLICK,btnClickHandler);
...
//			_reliveYuanBaoBtn.removeEventListener(MouseEvent.CLICK,btnClickHandler);
			...
			_reliveBtn4.removeEventListener(MouseEvent.CLICK,btnClickHandler);
		...
		//			{
		//				CrystalWarReliveSocketHandler.send((Math.floor((4-2+1)*Math.random()+2)));
		//			}
		//		}
		...
		private function timerCompleteHandler(evt:TimerEvent):void
		{
			...
			CrystalWarReliveSocketHandler.send((Math.floor((5-2+1)*Math.random()+2)));
			_sendTime = getTimer();
			dispose();
		}
...
//			{
//				CrystalWarReliveSocketHandler.send(1);
//				_sendTime = getTimer();
//				return;
//			}
...
//					break;
				case _reliveBtn1:
					CrystalWarReliveSocketHandler.send(2);
					break;
				case _reliveBtn2:
					CrystalWarReliveSocketHandler.send(3);
					break;
				case _reliveBtn3:
					CrystalWarReliveSocketHandler.send(4);
					break;
				case _reliveBtn4:
					CrystalWarReliveSocketHandler.send(5);
					break;
			}
			_sendTime = getTimer();
			dispose();
