...
%% io:format("Server_no: /~p/~n",[Server_no]),	
	if Key =/= "id"	->
			Id = emongo:findAndModify(tool:to_list(?MASTER_POOLID),
									  tool:to_list(Collection), 
									  "", 
									  tool:to_list(Table_name), 
									  tool:to_list(Key)),
			case Id of 
					undefined -> 0;
				_ -> Id
	   ...
	   true ->
			...
			case Id of 
					undefined -> 0;
				_ -> tool:to_integer(Server_no) * 10000000 + Id
			end			
	end.

%%插入数据时会先检查哪些域没有数据，然后添加相应的域，默认值，mongo不能处理默认值
insert(Table_name, FieldValueList) ->
%% io:format("insert_1_/~p/~n",[Table_name]),
	stat_db_access(Table_name, insert),
	IdValue = db_mongo:findAndModify("auto_ids", Table_name, "id"),
	KeyValuesList = [{id,IdValue}]++FieldValueList,
	FullKeyValuelist = fullKeyValue(Table_name,KeyValuesList),
	Opertion = db_mongoutil:make_insert_opertion(FullKeyValuelist), 
	emongo:insert(tool:to_list(?MASTER_POOLID),tool:to_list(Table_name),Opertion),  
    IdValue.
	
%% 插入数据并添加id ,需手动添加默认值
insert(Table_name, FieldList, ValueList) ->	
%% io:format("insert_2_/~p/~n",[Table_name]),	
  stat_db_access(Table_name, insert),
  Id = [id],
  IdValue = db_mongo:findAndModify("auto_ids", Table_name, "id"), 
...
fullKeyValue(Table_name,KeyValuesList) ->
	 ...
	 F = fun({Key, DefaultValue, FieldShort}) ->
				 case lists:keyfind(Key, 1, KeyValuesList) of
					 {_Key, _Vlaue} -> 
						 {};
					 _ -> 
						 {Key, DefaultValue} 
	 ...
	 L = [F({Key, DefaultValue, FieldShort})||{Key, DefaultValue, FieldShort}<- FieldsDefaultValueList],
	 RestList = lists:filter(fun(T)-> T =/= {} end, L),
	 KeyValuesList++RestList.

%% 删除数据
delete(Table_name, Where_List) ->
%% io:format("delete_1_/~p/~n",[Table_name]),	
	stat_db_access(Table_name, delete),
	[Opertion] = db_mongoutil:make_delete_opertion(Where_List),
	emongo:delete(atom_to_list(?MASTER_POOLID),atom_to_list(Table_name),Opertion),
	1.
...
select_count(Table_name, Where_List) ->
	Data = emongo:count(tool:to_list(?MASTER_POOLID),tool:to_list(Table_name),Where_List),
...
select_count(PoolId, Table_name, Where_List) ->
	Data = emongo:count(tool:to_list(db_mongoutil:get_slave_mongo_poolId(PoolId)),tool:to_list(Table_name),Where_List),
	case Data == undefined of
		true -> [0];
		_ -> [Data]
	end.
...
get_all_fieldsDefaultList(Table_name) ->
	...
	KeyList = [Key || {Key, _DefaultValue, FieldShort} <- FieldsValueList],
	DefaultValueList = [DefaultValue || {_Key, DefaultValue, FieldShort} <- FieldsValueList],
	{KeyList,DefaultValueList}.

get_all_fields(Table_name) ->
	FieldsValueList =  lib_player_rw:get_table_fields(Table_name),
	[Key || {Key, _DefaultValue, FieldShort} <- FieldsValueList].
...
%% io:format("select_one_1_/~p/~n",[Table_name]),	
...
%% stat_db_access(Table_name, select),
	[WhereOpertion,FieldOpertion] = db_mongoutil:make_select_opertion(transfer_fields(Table_name,Fields_sql_string), Where_List, Orderby_sql_List, Limit_sql_List),
	Result = emongo:find_all(tool:to_list(db_mongoutil:get_slave_mongo_poolId(PoolId)),tool:to_list(Table_name),WhereOpertion,FieldOpertion),
	R = 
		case Result of
			[] -> [];
			_ ->
...
%%主数据库中取数据
select_row(Table_name, Fields_sql_string, Where_List, Orderby_sql_List, Limit_sql_List) ->
%% io:format("select_row_1_/~p/~n",[Table_name]),	
	stat_db_access(Table_name, select),
	[WhereOpertion,FieldOpertion] = db_mongoutil:make_select_opertion(transfer_fields(Table_name,Fields_sql_string), Where_List, Orderby_sql_List, Limit_sql_List),
	Result = emongo:find_all(tool:to_list(?MASTER_POOLID),tool:to_list(Table_name),WhereOpertion,FieldOpertion),
	R = 
...
%%从数据库中取数据
select_row(PoolId, Table_name, Fields_sql_string, Where_List, Orderby_sql_List, Limit_sql_List) ->
%% io:format("select_row_1_/~p/~n",[Table_name]),	
%% stat_db_access(Table_name, select),
	[WhereOpertion,FieldOpertion] = db_mongoutil:make_select_opertion(transfer_fields(Table_name,Fields_sql_string), Where_List, Orderby_sql_List, Limit_sql_List),
	Result = emongo:find_all(tool:to_list(db_mongoutil:get_slave_mongo_poolId(PoolId)),tool:to_list(Table_name),WhereOpertion,FieldOpertion),
			...
			case Size of
				...
				_ ->
					F = fun(P) ->
								...
								case Value of
									undefined -> transfer_default_result(Table_name,tool:to_atom(P),undefined);
									_ -> 
										Value1 = float_to_integer(Value),
...
%% %% 特殊处理——1（获取财富排行, coin+bcoin） 
%% select_all_coin(Table_name, Fields_sql_string, Where_List, Orderby_sql_List, Limit_sql_List) ->
%% 	stat_db_access(Table_name, select), 
%% 	[WhereOpertion,FieldOpertion] = db_mongoutil:make_select_opertion(transfer_fields(Table_name,Fields_sql_string), Where_List, Orderby_sql_List, Limit_sql_List),
%% 	L = emongo:find_all(tool:to_list(?MASTER_POOLID),tool:to_list(Table_name),WhereOpertion,FieldOpertion),
%% 	L.
...
%% io:format("update_1_/~p/~n",[Table_name]),	
	%%启动更新coin_sum字段进程 
%% 	case Table_name == player andalso 
%% 				(lists:keyfind(coin, 1, Fields_sql_List)  =/= false 
%% 					orelse lists:keyfind(bcoin, 1, Fields_sql_List) =/= false ) of
%% 		false -> skip;
%% 		true ->  spawn(?MODULE,coin_sum_process,[Table_name, Fields_sql_List, Where_List])
%% 	end,
	stat_db_access(Table_name, update),
	[WhereOpertion,Fieldopertion] = db_mongoutil:make_update_opertion(Fields_sql_List, Where_List),
	emongo:update(tool:to_list(?MASTER_POOLID),tool:to_list(Table_name),WhereOpertion,Fieldopertion),
	1.
...
%% io:format("update_2_/~p/~n",[Table_name]),	
	stat_db_access(Table_name, update),
	%%启动更新coin_sum字段进程 
%% 	case Table_name == player andalso 
%% 				(lists:member(coin,Fields_sql_List)  =/= false 
%% 					orelse lists:member(bcoin, Fields_sql_List) =/= false) of
%% 		false -> skip;
%% 		true -> spawn(?MODULE,coin_sum_process,[Table_name, Fields_sql_List, [{Fieldstring,FieldValue}]])
%% 	end,
...
%% io:format("replace_1_/~p/~n",[Table_name]),	
	stat_db_access(Table_name, replace),
	[_WhereOpertion,Fieldopertion] = db_mongoutil:make_replace_opertion(Field_Value_List),
	Opertion = db_mongoutil:make_insert_opertion(Field_Value_List),
	{Index,Value} =  lists:nth(1,Field_Value_List),
	case isExistId(Table_name,Index,Value) of
		false -> %%插入数据
			emongo:insert(atom_to_list(?MASTER_POOLID),atom_to_list(Table_name),Opertion);
		true -> %%更新数据
			emongo:update(atom_to_list(?MASTER_POOLID),atom_to_list(Table_name),[{tool:to_binary(tool:to_list(Index)),Value}],Fieldopertion)
	end,	
	1.
...
isExistColumn(OldField,R) ->
	Bool = lists:keymember(list_to_binary(tool:to_list(OldField)),1,R).
...
sum(Table_name, Fields_sql_string, Where_List) ->
	Result = select_all(Table_name, Fields_sql_string, Where_List),
	case Result of
		[] -> 0;
		_ ->
