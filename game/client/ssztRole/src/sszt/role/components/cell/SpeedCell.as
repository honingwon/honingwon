package sszt.role.components.cell
{
	...
	public class SpeedCell extends BaseCell
	{
		private var _item:ItemInfo;
		private var _countLabel:TextField;
		private var _enabled:Boolean;
		public function SpeedCell()
		{
			super();
//			var _bg:Bitmap = new Bitmap(CellCaches.getCellBg());
//			addChild(_bg);_border = new Bitmap(new CellBorderAsset());
		...
		{
			_countLabel.text = String(value>0?value:0);
		}
		...
		public function set itemInfo(item:ItemInfo):void
		{
			...
			if(_item)
			{
				info = item.template;
				if(info==null)
					info = ItemTemplateList.getTemplate(206088);
...
//				else
//				{
					_countLabel.text = String(_item.count>0?_item.count:"0");
//				}	
			}else 
			{
//				info = null;
		...
		{
			_figureBound = new Rectangle(1,1,36,36);
		}
