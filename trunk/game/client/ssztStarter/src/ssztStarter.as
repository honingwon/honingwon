package
{
	...
	public class ssztStarter extends Sprite
	{
...
//			调试输入
			...
			function keyUpHandler(evt:KeyboardEvent):void
			{
				if(evt.keyCode == Keyboard.ENTER)
				{
					...
					ssztPrepare(_nameInput.text, site,1,tick,_isYellowVipInput.text,_isYellowYearVipInput.text,_yellowVipLevelInput.text,_isYellowHighVipInput.text, MD5.hash(_nameInput.text + site + "0" + tick + "qq-zone-710-5000-cm3d"));
...
//			trace(p);
		...
		{
			var t:SsztPrepare = new SsztPrepare(user,site,serverid,tick,sign,"1","","","",0,"119.147.19.43",_ipInput.text,int(_portInput.text),"0",int(isYellowVipInput),int(isYellowYearVipInput),int(yellowVipLevelInput),int(isYellowHighVipInput),prepareComplete);
			t.setup(this);
		}
