package sszt.scene.components.ActivityIcon
{
	 ...
	 * 
	 */	
	public class PayTagView extends BaseIconView
	{
...
//			_index = 17;
		...
		public static function getInstance():IActivityView
		{
			if(instance == null)
			{
				instance = new PayTagView() as IActivityView;
			}
			return instance;
		}
...
//			addChild(_countDown);
			...
			_btn = new MAssetButton1(AssetUtil.getAsset("ssztui.scene.TopBtnPayAsset") as MovieClip);
		...
		override protected function initEvent():void
		{
			...
			_btn.addEventListener(MouseEvent.MOUSE_OVER,overHandler);
			_btn.addEventListener(MouseEvent.MOUSE_OUT,outHandler);
			ModuleEventDispatcher.addModuleEventListener(ActivityEvent.GET_AWARD,getPayTag);
			ModuleEventDispatcher.addModuleEventListener(ActivityEvent.GET_OPEN_SERVER_DATA,getPayTagAct);
		...
		private function getPayTag(evt:ModuleEvent):void
		{
			setisEffectShow();
		}
		
		private function getPayTagAct(evt:ModuleEvent):void
		{
			_stopTime = getOpenServerTime();
			setisEffectShow();
		}
		...
		private function onClick(e:MouseEvent):void
		{
			_isLogin = 0;
			initData();
			SetModuleUtils.addPayTagView();
		}
		
		
		private function overHandler(e:MouseEvent):void
		{
			TipsUtil.getInstance().show(LanguageManager.getWord("ssztl.activity.listLabel1"),null,new Rectangle(e.stageX,e.stageY,0,0));
		}
		...
		override protected function removeEvent():void
		{
			...
			ModuleEventDispatcher.removeModuleEventListener(ActivityEvent.GET_AWARD,getPayTag);
			ModuleEventDispatcher.removeModuleEventListener(ActivityEvent.GET_OPEN_SERVER_DATA,getPayTagAct);
