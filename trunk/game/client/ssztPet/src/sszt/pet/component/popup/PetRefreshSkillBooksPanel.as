package sszt.pet.component.popup
{
	...
	import sszt.pet.component.items.PetSkillBookItemView;
	import sszt.pet.event.PetEvent;
	import sszt.pet.mediator.PetMediator;
	import sszt.pet.socketHandler.PetGetSkillBookListSocketHandler;
	import sszt.pet.socketHandler.PetRefreshSkillBooksSocketHandler;
	...
	public class PetRefreshSkillBooksPanel extends MPanel
	{
		...
		/**
		 * PetSkillBookItemView
		 * */
		private var _bookItemList:Array;
		private var _tile:MTile;
		
		public function PetRefreshSkillBooksPanel(mediator:PetMediator)
		{
			super(new MCacheTitle1("", new Bitmap(new TitleSkillAsset())), true,-1);
			_mediator = mediator;
			_mediator.module.petsInfo.initPetRefreshSkillBooksInfo();
			updateMoneyInfo(null);
			_luckyImgCache = new BarAssetLP();
			initView();
			addEvent();
			
			PetGetSkillBookListSocketHandler.send();
		}
		...
		private function initView():void
		{
			...
			_bookItemList = [];
			var tmpBooksItem:PetSkillBookItemView;
			for(var i:int = 0; i < BOOKS_AMOUNT_MAX; i++)
			{
				tmpBooksItem = new PetSkillBookItemView(_mediator);
		...
		private function addEvent():void
		{
			...
			_mediator.module.petsInfo.petRefreshSkillBooksInfo.addEventListener(PetEvent.GET_SKILL_BOOK_LIST, update);
			_mediator.module.petsInfo.petRefreshSkillBooksInfo.addEventListener(PetEvent.GET_SKILL_BOOK_SUCCESSED, getSkillBookSuccessedHandler);
		...
		private function removeEvent():void
		{
			...
			_mediator.module.petsInfo.petRefreshSkillBooksInfo.removeEventListener(PetEvent.GET_SKILL_BOOK_LIST, update);
			_mediator.module.petsInfo.petRefreshSkillBooksInfo.removeEventListener(PetEvent.GET_SKILL_BOOK_SUCCESSED, getSkillBookSuccessedHandler);
		...
		{
			updateLuckyValue();
			var gotBooksPlace:int = _mediator.module.petsInfo.petRefreshSkillBooksInfo.gotbooksPlace;
			var bookItem:PetSkillBookItemView;
			for(var i:int = 0; i < BOOKS_AMOUNT_MAX; i++)
			{
				bookItem = PetSkillBookItemView(_bookItemList[i]);
				if(!bookItem.visible)
				{
					continue;
				}
		...
		{
			clearBooks();
			var list:Array = _mediator.module.petsInfo.petRefreshSkillBooksInfo.booksList;
			for(var i:int = 0; i < list.length; i++)
			{
				(_bookItemList[i] as PetSkillBookItemView).info = list[i];
			}
		}
		...
		private function clearBooks():void
		{
			...
			{
				(_bookItemList[i] as PetSkillBookItemView).info = null;
			}
		}
		
		private function updateLuckyValue():void
		{
			_luckyValue = _mediator.module.petsInfo.petRefreshSkillBooksInfo.lucyValue;
		...
		{
			if(_mediator.module.petsInfo.petRefreshSkillBooksInfo.lucyValue>0)
			{
		...
		private function sendRefreshOnceRequest():void
		{
			...
			{
				PetRefreshSkillBooksSocketHandler.send(1);
			}
			...
			{
				PetRefreshSkillBooksSocketHandler.send(0);
			}
			else
			{
		...
		private function sendRefreshRequest():void
		{
			...
			{
				PetRefreshSkillBooksSocketHandler.send(3);
			}
			...
			{
				PetRefreshSkillBooksSocketHandler.send(2);
			}
			else
			{
				QuickTips.show(LanguageManager.getWord('ssztl.common.bindYuanBaoNotEnoughShort'));
		...
		override public function dispose():void
		{
			removeEvent();
			super.dispose();
			_mediator.module.petsInfo.clearPetRefreshSkillBooksInfo();
			_mediator = null;
			...
			for(var i:int = 0; i < _bookItemList.length; i++)
			{
				(_bookItemList[i] as PetSkillBookItemView).dispose();
