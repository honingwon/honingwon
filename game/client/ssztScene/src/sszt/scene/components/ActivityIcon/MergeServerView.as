package sszt.scene.components.ActivityIcon
{
	 ...
	 * 
	 */	
	public class MergeServerView extends BaseIconView
	{
		...
		private var _endTime:int = 0388159999;
		/**
		 * 是否开启按钮 
		 */
		private static var _isShow:Boolean = true;
		
		public function MergeServerView()
		{
			super();
			init();
			initEvent();
		...
		public static function getInstance():IActivityView
		{
			if(instance == null)
			{
				instance = new MergeServerView() as IActivityView;
			}
			return instance;
		}
		...
		{
			_btn = new MAssetButton1(AssetUtil.getAsset("ssztui.scene.TopBtnMidAutumnAsset") as MovieClip);
		...
		{
			_endTime = (OpenActivityTemplateList.activityTypeRecArray4[0] as OpenActivityTemplateListInfo).end_time
		...
		private function onClick(e:MouseEvent):void
		{
			...
			SetModuleUtils.addMergeServer();
		}
		
		private function overHandler(e:MouseEvent):void
		{
			TipsUtil.getInstance().show(LanguageManager.getWord("ssztl.mergeServer.mergeServer"),null,new Rectangle(e.stageX,e.stageY,0,0));
		}
