package sszt.scene.components.ActivityIcon
{
	 ...
	 * 
	 */	
	public class ActivityThievesView extends BaseIconView
	{
		 ...
		 *活动id 
		 */
		private var _pvpActiveId:int = 1002;
		...
		*/
		...
		public function ActivityThievesView()
		{
			super();
//			_index = 16;
			init();
		...
		public static function getInstance():IActivityView
		{
			if(instance == null)
			{
				instance = new ActivityThievesView() as IActivityView;
			}
			return instance;
		}
...
//			}
		...
		private function init():void
		{
			...
			_btn = new MAssetButton1(AssetUtil.getAsset("ssztui.scene.TopBtnActiveZZAsset") as MovieClip);
			addChild(_btn);
		...
		override protected function initEvent():void
		{
			...
			_countDown.addEventListener(Event.COMPLETE,completeTh);
			ModuleEventDispatcher.addModuleEventListener(ActiveStartEvents.ACTIVE_START_UPDATE,updateActivityThievesTime);
		}
		...
		override protected function removeEvent():void
		{
			...
			_countDown.removeEventListener(Event.COMPLETE,completeTh);
			ModuleEventDispatcher.removeModuleEventListener(ActiveStartEvents.ACTIVE_START_UPDATE,updateActivityThievesTime);
		}
		...
		private function updateActivityThievesTime(evt:ModuleEvent):void
		{
			setData();
		}
		...
		private function completeTh(evt:Event):void
		{
			_effect.visible = true;
			_countDown.visible = false;
		}
		...
		{
			TipsUtil.getInstance().show(LanguageManager.getWord("ssztl.activity.ActivityThieves"),null,new Rectangle(e.stageX,e.stageY,0,0));
		}
