package sszt.rank.components.views.mount
{
	...
	import sszt.rank.components.itemView.mount.MountFightRankItemView;
	...
	public class MountQualityRankView extends RankViewImpl
	{
		...
		private var _curItemView:MountQualityRankView;
		
		private var _colWidth:Array = [51,130,174,110];
		
		public function MountQualityRankView(rankInfos:RankInfo)
		{
		...
		private function initView():void
		{
			...
			addChild(MBackgroundLabel.getDisplayObject(new Rectangle(colX[0]+2,3,_colWidth[1],16),new MAssetLabel(LanguageManager.getWord("ssztl.common.mountName"),MAssetLabel.LABEL_TYPE_TITLE2)));
			...
			addChild(MBackgroundLabel.getDisplayObject(new Rectangle(colX[2]+2,3,_colWidth[3],16),new MAssetLabel(LanguageManager.getWord("ssztl.common.mountQuality"),MAssetLabel.LABEL_TYPE_TITLE2)));
		...
		public function setData():void{
			...
			_resultViewList = [];
			var i:int;
			var itemView:MountFightRankItemView;
			var flag:Boolean;
			for(i = 0; i < _itemInfoList.length; i++)
			{
				itemView = new MountFightRankItemView(_itemInfoList[i]);
		...
		{
			curItemChange(evt.currentTarget as MountFightRankItemView);
		}
		
		private function curItemChange(item:MountFightRankItemView):void
		{
			item.select = true;
		}
		...
		override public function dispose():void
		{
			...
			if(_resultViewList)
			{
				for each(var itemView:MountFightRankItemView in _resultViewList)
				{
					itemView.dispose();
					itemView = null;
				}
