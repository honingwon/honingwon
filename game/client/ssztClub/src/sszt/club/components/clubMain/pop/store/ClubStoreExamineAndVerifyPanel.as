package sszt.club.components.clubMain.pop.store
{
	...
	import sszt.club.socketHandlers.ClubStoreExamineAndVerifySocketHandler;
	...
	public class ClubStoreExamineAndVerifyPanel extends MPanel
	{
		...
		public function ClubStoreExamineAndVerifyPanel(mediator:ClubMediator)
		{
			_mediator = mediator;
//			super(new MCacheTitle1("",new Bitmap(new ClubStoreTitleAsset())),true,-1);
			super(new MCacheTitle1(LanguageManager.getWord("ssztl.club.examineAndVerify")),true,-1);
			initView();
			initEvent();
		...
		private function initView():void
		{
			...
			_bgTxtTip = new MAssetLabel(LanguageManager.getWord('ssztl.club.appliedItemRecordsTip'),MAssetLabel.LABEL_TYPE_TAG,TextFieldAutoSize.LEFT);
			...
			{
				var record:ClubStoreExamineAndVerifyRecord = new ClubStoreExamineAndVerifyRecord(_mediator);
		...
		private function initEvent():void
		{
			...
			_mediator.clubInfo.clubStoreInfo.addEventListener(ClubStoreUpdateEvent.EXAMINE_AND_VERIFY_UPDATE, examineAndVerifyUpdateHandler);
		}		
		...
		private function removeEvent():void
		{
			...
			{
				_mediator.clubInfo.clubStoreInfo.removeEventListener(ClubStoreUpdateEvent.EXAMINE_AND_VERIFY_UPDATE, examineAndVerifyUpdateHandler);
			}
		}
		...
		private function pageChangeHandler(event:Event):void
		{
			...
			ClubStoreExamineAndVerifySocketHandler.send(_currentPage);
		}
		
		private function examineAndVerifyUpdateHandler(event:Event):void
		{
			clearRecordsView();
			var list:Array = _mediator.clubInfo.clubStoreInfo.examineAndVerify;
			var total:int = _mediator.clubInfo.clubStoreInfo.examineAndVerifyTotal;
			for(var i:int = 0; i < list.length; i++)
			{
				(_records[i] as ClubStoreExamineAndVerifyRecord).recordInfo = list[i];
			}
		...
		private function clearRecordsView():void
		{
			...
			{
				if( (_records[i] as ClubStoreExamineAndVerifyRecord).recordInfo )
				{
					(_records[i] as ClubStoreExamineAndVerifyRecord).recordInfo = null;
				}
			}
		}
