...
Calendar.prototype.draw = function() {
    ...
    mvAry[mvAry.length] = '    <table width="100%" border="0" cellpadding="0" cellspacing="1" style="margin:0 0;font-size:12px;">';
    mvAry[mvAry.length] = '      <tr style="padding:0 0 0 0;height:0;">';
    mvAry[mvAry.length] = '        <th align="left" width="1%" style="padding:0 0 0 0;height:0;"><input style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:16px;height:20px;';
    ...
    mvAry[mvAry.length] = '        <th align="center" width="98%" nowrap="nowrap" style="padding:0 0 0 0;height:0;"><select name="calendarYear" id="calendarYear" style="font-size:12px;"></select><select name="calendarMonth" id="calendarMonth" style="font-size:12px;';
    if (calendar.DateMode > pickMode["month"]) { mvAry[mvAry.length] = 'display:none;'; } //pickMode 精确到年时隐藏“月”
    mvAry[mvAry.length] = '"></select></th>';
    mvAry[mvAry.length] = '        <th align="right" width="1%" style="padding:0 0 0 0;height:0;"><input style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:16px;height:20px;';
    if (calendar.DateMode > pickMode["month"]) { mvAry[mvAry.length] = 'display:none;'; } //pickMode 精确到年时隐藏“月”
    mvAry[mvAry.length] = '" name="nextMonth" type="button" id="nextMonth" value="&gt;" /></th>';
    mvAry[mvAry.length] = '      </tr>';
    mvAry[mvAry.length] = '    </table>';
    mvAry[mvAry.length] = '    <table id="calendarTable" width="100%" style="margin:0 0;border:0px solid #CCCCCC;background-color:#FFFFFF;font-size:12px;';
    ...
    mvAry[mvAry.length] = '      <tr style="padding:0 0 0 0;height:0;">';
    for (var i = 0; i < 7; i++) {
        mvAry[mvAry.length] = '      <th style="padding:0 0 0 0;height:0;font-weight:normal;background-color:' + calendar.colors["tr_bg"] + ';color:' + calendar.colors["tr_word"] + ';">' + Calendar.language["weeks"][this.lang][i] + '</th>';
    }
    mvAry[mvAry.length] = '      </tr>';
    for (var i = 0; i < 6; i++) {
        mvAry[mvAry.length] = '    <tr align="center" style="padding:0 0 0 0;height:0;">';
        for (var j = 0; j < 7; j++) {
            if (j == 0) {
                mvAry[mvAry.length] = '  <td style="padding:0 0 0 0;height:0;cursor:default;color:' + calendar.colors["sun_word"] + ';"></td>';
            } else if (j == 6) {
                mvAry[mvAry.length] = '  <td style="padding:0 0 0 0;height:0;cursor:default;color:' + calendar.colors["sat_word"] + ';"></td>';
            } else {
                mvAry[mvAry.length] = '  <td style="padding:0 0 0 0;height:0;cursor:default;"></td>';
            }
        }
        mvAry[mvAry.length] = '    </tr>';
    }
    ...
    mvAry[mvAry.length] = '"><td align="center" colspan="7" style="padding:0 0 0 0;height:0;">';
    ...
    mvAry[mvAry.length] = '        <input name="calendarClear" type="button" id="calendarClear" value="' + Calendar.language["clear"][this.lang] + '" style="margin:0 0;border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:40px;height:20px;font-size:12px;cursor:pointer;"/>';
    ...
    mvAry[mvAry.length] = '" style="margin:0 0;border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:60px;height:20px;font-size:12px;cursor:pointer"/>';
    mvAry[mvAry.length] = '        <input name="calendarClose" type="button" id="calendarClose" value="' + Calendar.language["close"][this.lang] + '" style="margin:0 0;border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:40px;height:20px;font-size:12px;cursor:pointer"/>';
