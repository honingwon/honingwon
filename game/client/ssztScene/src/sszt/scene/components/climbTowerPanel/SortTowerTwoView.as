package sszt.scene.components.climbTowerPanel
{
	...
	public class SortTowerTwoView extends Sprite
	{
		...
		private const COPY_ID:int = 4200901;
		
		private const COST:int =  128;
		
		public function SortTowerTwoView(mediator:SceneMediator)
		{
			_mediator = mediator;
			_copyTemplateInfo = CopyTemplateList.getCopy(COPY_ID);
		...
		private function initView():void
		{
			...
			_picPath = GlobalAPI.pathManager.getBannerPath("coverTowerTwo.jpg");
		...
		private function resetCloseHandler(e:CloseEvent):void
		{
			if(e.detail == MAlert.OK)
			{
				CopyLimitNumResetSocketHandler.send(COPY_ID);
			}
		}
