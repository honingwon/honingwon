package sszt.scene.components.shenMoWar.personalWar.ranking
{
	...
	import sszt.scene.components.shenMoWar.ranking.ShenMoRankingItemView;
	...
	public class PerWarRankingPanel extends Sprite
	{
		...
		public function PerWarRankingPanel(argMediator:SceneMediator)
		{
			...
			initialEvents();
			//向服务器请求数据
			PerWarMemberListUpdateSocketHandler.send(_mediator.perWarInfo.warSceneId);
		}
		...
		private function initialView():void
		{
			...
			_myScoreLabel = new MAssetLabel("",MAssetLabel.LABELTYPE16);
			_myScoreLabel.mouseEnabled = _myScoreLabel.mouseWheelEnabled = false;
			_myScoreLabel.move(200,235);
			_container.addChild(_myScoreLabel);
...
//			_showBtn = new MBitmapButton(new ZoomBtnAsset());
			...
			{
				var tmpItemView:PerWarRankingItemView = new PerWarRankingItemView(_mediator);
		...
		private function initialEvents():void
		{
			...
			_mediator.perWarInfo.perWarMembersInfo.addEventListener(ScenePerWarUpdateEvent.PERWAR_MENBERS_LIST_UPDATE,getData);
		...
		private function removeEvents():void
		{
			...
			_mediator.perWarInfo.perWarMembersInfo.removeEventListener(ScenePerWarUpdateEvent.PERWAR_MENBERS_LIST_UPDATE,getData);
		...
		private function getData(e:ScenePerWarUpdateEvent):void
		{
			clearList();
			var tmpList:Array = _mediator.perWarInfo.perWarMembersInfo.getMemberListByPage(_pageView.currentPage,PAGE_COUNT);
			for(var i:int = 0;i < tmpList.length;i++)
			{
				if(_itemList[i])_itemList[i].info = tmpList[i];
			}
			_pageView.totalRecord = _mediator.perWarInfo.perWarMembersInfo.membersItemList.length;
			//更新自己信息
			var tmpMyInfo:PerWarMembersItemInfo = _mediator.perWarInfo.perWarMembersInfo.getMemberItemInfo(GlobalData.selfPlayer.userId);
			if(!tmpMyInfo)return;
			_myRankingLabel.text = tmpMyInfo.rankingNum.toString();
			_myNickLabel.text = "[" + tmpMyInfo.serverId + "]" + tmpMyInfo.playerNick;
			_myScoreLabel.text = tmpMyInfo.score.toString();
		}
		
		private function clearList():void
		{
			for each(var i:PerWarRankingItemView in _itemList)
			{
				i.info = null;
			}
		}
		...
		{
			removeEvents();
			if(!_mediator.sceneModule.perWarRewardsPanel)
			{
