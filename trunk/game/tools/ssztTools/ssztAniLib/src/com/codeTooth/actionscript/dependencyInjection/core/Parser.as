		...
		public function parse(diXML:XML):Dictionary/*of Item*/
		{
			...
			if(diXML.item != undefined)
			{
				...
				for each(var itemXML:XML in itemsList)
				{
					...
					item.isSingle = itemXML.@isSingle == undefined ? false : StringUtils.toBoolean(String(itemXML.@isSingle));
					item.isReinject = itemXML.@isReinject == undefined ? true : StringUtils.toBoolean(String(itemXML.@isReinject));
