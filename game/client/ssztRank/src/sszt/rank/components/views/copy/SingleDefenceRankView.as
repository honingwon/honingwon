package sszt.rank.components.views.copy
{
	...
	import sszt.rank.components.itemView.copy.SingleDefenceRankItemView;
	...
	public class SingleDefenceRankView extends RankViewImpl
	{
		private var _colWidth:Array = [60, 160, 80, 175];	//475
		...
		public function SingleDefenceRankView(rankInfos:RankInfo)
		{
			super();
		...
		private function initView():void
		{
			...
			addChild(MBackgroundLabel.getDisplayObject(new Rectangle(0,3,  _colWidth[0],16),new MAssetLabel(LanguageManager.getWord("ssztl.rank.rank"),MAssetLabel.LABEL_TYPE_TITLE2)));
			...
			addChild(MBackgroundLabel.getDisplayObject(new Rectangle(colX[1]+2,3,_colWidth[2],16),new MAssetLabel(LanguageManager.getWord("ssztl.rank.totalPass"),MAssetLabel.LABEL_TYPE_TITLE2)));
			addChild(MBackgroundLabel.getDisplayObject(new Rectangle(colX[2]+2,3,_colWidth[3],16),new MAssetLabel(LanguageManager.getWord("ssztl.common.time"),MAssetLabel.LABEL_TYPE_TITLE2)));
		...
		{
			var itemView:SingleDefenceRankItemView;
			...
			{
				itemView = new SingleDefenceRankItemView(list[i]);
		...
		{
			curItemChange(evt.currentTarget as SingleDefenceRankItemView);
		}
		
		private function curItemChange(item:SingleDefenceRankItemView):void
		{
			item.select = true;
		}
		...
		override public function dispose():void
		{
			...
			if(_resultViewList)
			{
				for each(var itemView:SingleDefenceRankItemView in _resultViewList)
				{
					itemView.dispose();	
					itemView = null;
				}
