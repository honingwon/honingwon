package sszt.rank.components.views.club
{
	...
	import sszt.rank.components.itemView.club.ClubRankItemView;
	...
	public class ClubRank1View extends RankViewImpl
	{
		...
		private var _curItemView:ClubRankItemView;
		
		private var _colWidth:Array = [51,130,174,110];
		
		public function ClubRank1View(rankInfos:RankInfo)
		{
		...
		private function initView():void
		{
			...
			addChild(MBackgroundLabel.getDisplayObject(new Rectangle(colX[0]+2,3,_colWidth[1],16),new MAssetLabel(LanguageManager.getWord("ssztl.common.clubName"),MAssetLabel.LABEL_TYPE_TITLE2)));
			addChild(MBackgroundLabel.getDisplayObject(new Rectangle(colX[1]+2,3,_colWidth[2],16),new MAssetLabel(LanguageManager.getWord("ssztl.common.clubLevel"),MAssetLabel.LABEL_TYPE_TITLE2)));
			addChild(MBackgroundLabel.getDisplayObject(new Rectangle(colX[2]+2,3,_colWidth[3],16),new MAssetLabel(LanguageManager.getWord("ssztl.common.clubMoney"),MAssetLabel.LABEL_TYPE_TITLE2)));
		...
		public function setData():void{
			...
			_resultViewList = [];
			var i:int;
			var itemView:ClubRankItemView;
			var flag:Boolean;
			for(i = 0; i < _itemInfoList.length; i++)
			{
				itemView = new ClubRankItemView(_itemInfoList[i]);
		...
		//				if(list && list[i])
		//				{
		//					itemView = new ClubRankItemView(list[i]);
		//					itemView.addEventListener(MouseEvent.CLICK,itemClickHandler);
		//					_resultViewList.push(itemView);
		//					_tile.appendItem(itemView);
		...
		{
			curItemChange(evt.currentTarget as ClubRankItemView);
		}
		
		private function curItemChange(item:ClubRankItemView):void
		{
		...
		override public function dispose():void
		{
			...
			if(_resultViewList)
			{
				for each(var itemView:ClubRankItemView in _resultViewList)
				{
					itemView.dispose();
					itemView = null;
				}
