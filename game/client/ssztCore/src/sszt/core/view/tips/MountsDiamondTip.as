package sszt.core.view.tips
{
	...
	import sszt.core.data.mounts.MountsDiamondTemplate;
	import sszt.core.data.mounts.MountsDiamondTemplateList;
	import sszt.core.manager.LanguageManager;
	import sszt.ui.mcache.splits.MCacheSplit2Line;
	
	public class MountsDiamondTip extends BaseDynamicTip
	{
		public static var instance:MountsDiamondTip;//当前tips的一个实例
		private var _lineBg:Array;//线条背景
		private var _templateId:int;
		private var _diamond:int;
		
		
		public function MountsDiamondTip()
		{
			super();
			_lineBg = new Array();
		}
		
		public static function getInstance():MountsDiamondTip
		{
			if( instance == null )
			{
				instance = new MountsDiamondTip();
			}
		...
		//启动块
		public function showTip(template_id:int, diamond:int, pos:Point):void
		{
			//this.level = level;
		...
		public function getDataStr(template_id:int, diamond:int):String
		{
			this._templateId = template_id;
			this._diamond = diamond;
		...
		private function getProperty(d:int):String
		{
			...
			var mountsInfo:MountsDiamondTemplate = MountsDiamondTemplateList.getDiamondInfo(_templateId,d);
			if(mountsInfo)
			{
				if(mountsInfo.attack > 0) tipStr += PropertyType.getName(PropertyType.ATTR_ATTACK) + " +" +  mountsInfo.attack + "\n";
				if(mountsInfo.defence > 0) tipStr += PropertyType.getName(PropertyType.ATTR_DEFENSE) + " +" +  mountsInfo.defence + "\n";
				if(mountsInfo.hp > 0) tipStr += PropertyType.getName(PropertyType.ATTR_HP) + " +" +  mountsInfo.hp + "\n";
				if(mountsInfo.mp > 0) tipStr += PropertyType.getName(PropertyType.ATTR_MP) + " +" +  mountsInfo.mp;
			}
			return tipStr;
		}
		private function setTip():void
		{
			...
			addStringToField(LanguageManager.getWord("ssztl.mounts.attentionDiamondTip1"),getTextFormat(0xEDDB60));
			addStringToField(LanguageManager.getWord("ssztl.mounts.attentionDiamondTip2"),getTextFormat(0xEDDB60));
			show();
		}
		...
		private function currentInfo():void
		{
			...
			var mountsInfo:MountsDiamondTemplate = MountsDiamondTemplateList.getDiamondInfo(_templateId,_diamond);
			if(mountsInfo)
			{
				if(mountsInfo.attack > 0) addStringToField(PropertyType.getName(PropertyType.ATTR_ATTACK) + "：+" +  mountsInfo.attack,getTextFormat(tempColor));
				if(mountsInfo.defence > 0) addStringToField(PropertyType.getName(PropertyType.ATTR_DEFENSE) + "：+" +  mountsInfo.defence,getTextFormat(tempColor));
				if(mountsInfo.hp > 0) addStringToField(PropertyType.getName(PropertyType.ATTR_HP) + "：+" +  mountsInfo.hp,getTextFormat(tempColor));
				if(mountsInfo.mp > 0) addStringToField(PropertyType.getName(PropertyType.ATTR_MP) + "：+" +  mountsInfo.mp,getTextFormat(tempColor));
			}
		...
		private function nextInfo():void
		{
			...
			if (_diamond >= 10)
				return;
			var mountsInfo:MountsDiamondTemplate = MountsDiamondTemplateList.getDiamondInfo(_templateId,(_diamond + 1));
			if(mountsInfo)
			{
				if(mountsInfo.attack > 0) addStringToField(PropertyType.getName(PropertyType.ATTR_ATTACK) + "：+" +  mountsInfo.attack,getTextFormat(tempColor));
				if(mountsInfo.defence > 0) addStringToField(PropertyType.getName(PropertyType.ATTR_DEFENSE) + "：+" +  mountsInfo.defence,getTextFormat(tempColor));
