...
-export([whereis_name/1, 
		 ...
		 monster_process_name/2, 
		 player_process_accountname/1, 
		 ...
		 player_process_site_and_username/2,
		 collect_process_name/2,
 		 string_width/1,
		 name_ver/1
]).
...
%% 怪物进程
monster_process_name(MapId,MonsterId) when is_integer(MonsterId) andalso is_integer(MapId) ->
	lists:concat([m_p_, MapId, m_, MonsterId]);
monster_process_name(MapId,MonsterId) when is_atom(MonsterId) andalso is_atom(MapId) ->
	lists:concat([m_p_,MapId,m_,MonsterId]);
monster_process_name(MapId,MonsterId) when is_list(MonsterId) andalso is_list(MapId) ->
	L1 = lists:flatten(["m_p_"|MapId]),
	L2 = lists:flatten([L1|"m_"]),
	lists:flatten([L2|MonsterId]);
monster_process_name(MapId,MonsterId) when is_binary(MonsterId) andalso is_binary(MapId) ->
	lists:concat([m_p_,tool:md5(MapId),m_,tool:md5(MonsterId)]).

%% 采集物品进程
collect_process_name(MapId, CollectId) when is_integer(CollectId) andalso is_integer(MapId) ->
	lists:concat([m_p_, MapId, c_, CollectId]);
collect_process_name(MapId, CollectId) when is_atom(CollectId) andalso is_atom(MapId) ->
	lists:concat([m_p_, MapId, c_, CollectId]);
collect_process_name(MapId, CollectId) when is_list(CollectId) andalso is_list(MapId) ->
	L1 = lists:flatten(["m_p_"|MapId]),
	L2 = lists:flatten([L1|"m_"]),
	lists:flatten([L2|CollectId]);
collect_process_name(MapId, CollectId) when is_binary(CollectId) andalso is_binary(MapId) ->
	lists:concat([m_p, tool:md5(MapId), m_, tool:md5(CollectId)]).
...
%% 站点和用户名唯一性
player_process_site_and_username(Site, UserName) ->
...
get_https_content(Method, Request, HTTPOptions, Options) ->
