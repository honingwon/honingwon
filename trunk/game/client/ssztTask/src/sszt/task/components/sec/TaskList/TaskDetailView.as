package sszt.task.components.sec.TaskList
{
	...
	public class TaskDetailView extends MScrollPanel
	{
...
//		private var _deployList:Vector.<DeployItemInfo>;
		...
		public function TaskDetailView()
		{
			super();
			init();
		}
...
//			_giveupBtn = new MCacheAsset1Btn(2,LanguageManager.getWord("ssztl.task.giveUpTask"));
			
			_giveupBtn = new MCacheAssetBtn1(0,3,LanguageManager.getWord("ssztl.task.giveUpTask"));
			_giveupBtn.move(119, 302);
			addChild(_giveupBtn);
			_giveupBtn.enabled = false;
		}
		
		private function initEvent():void
		{
			addEventListener(TextEvent.LINK,textLinkClickHandler);
			_giveupBtn.addEventListener(MouseEvent.CLICK,btnClickHandler);
		...
		private function removeEvent():void
		{
			...
			_giveupBtn.removeEventListener(MouseEvent.CLICK,btnClickHandler);
...
//			MAlert.show("您确定放弃" + _info.getTemplate().title + "任务吗？\n放弃将不能获得丰厚的任务奖励！",LanguageManager.getWord("ssztl.common.alertTitle"),MAlert.OK | MAlert.CANCEL,null,closeHandler);
			MAlert.show(LanguageManager.getWord("ssztl.task.isSureGiveUpTask",_info.getTemplate().title),LanguageManager.getWord("ssztl.common.alertTitle"),MAlert.OK | MAlert.CANCEL,null,closeHandler);
		}
		...
		{
			var taskInfo:TaskItemInfo = e.currentTarget as TaskItemInfo;
		...
		public function set info(value:TaskItemInfo):void
		{
...
//			if(_info.getCurrentState().exploit > 0)awards += "个人功勋：" + _info.getCurrentState().exploit;
			...
			if(_info.getCurrentState().awardYuanBao > 0)awards +=  LanguageManager.getWord("ssztl.common.yuanBao") + "：<font color='#00cc00'>" + _info.getCurrentState().awardYuanBao + "　　</font>";
...
//			_awardList = new Vector.<TaskAwardCell>();
		...
		public function get info():TaskItemInfo
		{
			return _info;
		}
