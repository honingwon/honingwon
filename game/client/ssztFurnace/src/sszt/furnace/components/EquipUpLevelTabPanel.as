package sszt.furnace.components
{
	...
	import sszt.core.data.furnace.PromotionItemInfo;
	...
	public class EquipUpLevelTabPanel extends BaseFurnaceTabPanel
	{
		private var _upLevelBtn:MCacheAssetBtn1;
...
//		public static const LABEL_FORMAT:TextFormat = new TextFormat(LanguageManager.getWord("ssztl.common.wordType"),12,null,null,null,null,null,null,TextFormatAlign.LEFT,null,null,null,4);
		...
		/**装备升级材料**/
		public static const EQUIPUPLEVEL_MATERIAL_CATEGORYID_LIST:Array = [CategoryType.UPGRADE, CategoryType.UPGRADE_UPLEVEL];
...
//		private var _stoneCell:FurnaceBaseCell;
		...
		private var _uplevelEffect:BaseLoadEffect;
		private var _successEffect:MovieClip;
		
		private var _lableArray:Array;
		
		public function EquipUpLevelTabPanel(mediator:FurnaceMediator)
		{
			super(mediator);
		}
		...
		override protected function init():void
		{
			...
			_upLevelBtn = new MCacheAssetBtn1(2,0,LanguageManager.getWord("ssztl.furnace.startUpgrade"));
			_upLevelBtn.enabled = false;
			_upLevelBtn.move(115,269);
			addChild(_upLevelBtn);
		}
		...
		override public function show():void
		{
			super.show();
			/**加载数据到快速购买**/
			furnaceInfo.currentBuyType = FurnaceBuyType.UPLEVEL;
			/**加载数据到另外两个面板**/
			furnaceInfo.initialFurnaceVector(qualityItemListChecker,EQUIPUPLEVEL_MATERIAL_CATEGORYID_LIST);
			/**监听背包变化**/ 
			GlobalData.bagInfo.addEventListener(BagInfoUpdateEvent.ITEM_ID_UPDATE,bagItemUpdateHandler);
			_upLevelBtn.addEventListener(MouseEvent.CLICK,transformBtnHandler);
			furnaceModule.furnaceInfo.addEventListener(FuranceEvent.UPLEVEL_SUCCESS,uplevelHandler);
		...
		override public function hide():void
		{
			...
			_upLevelBtn.removeEventListener(MouseEvent.CLICK,transformBtnHandler);
			furnaceModule.furnaceInfo.removeEventListener(FuranceEvent.UPLEVEL_SUCCESS,uplevelHandler);
...
//			_equipBg.bitmapData = AssetUtil.getAsset("ssztui.furnace.LightCellBgAsset" ,BitmapData) as BitmapData;
		...
		private function uplevelHandler(evt:FuranceEvent):void
		{
			if(!_successEffect)
			{
				_successEffect =  AssetUtil.getAsset("ssztui.furnace.EffectFurnaceUplevelAsset",MovieClip) as MovieClip;
			...
			if(!_uplevelEffect)
			{
				_uplevelEffect = new BaseLoadEffect(MovieTemplateList.getMovie(EffectType.STRENGTHEN_EFFECT)); //UPQUALITY_EFFECT
				_uplevelEffect.move(155,211);
				_uplevelEffect.addEventListener(Event.COMPLETE,uplevelCompleteHandler);
				_uplevelEffect.play(SourceClearType.TIME,300000);
				addChild(_uplevelEffect);
			}
			*/
		}
		...
		private function uplevelCompleteHandler(evt:Event):void
		{
			_uplevelEffect.removeEventListener(Event.COMPLETE,uplevelCompleteHandler);
			_uplevelEffect.dispose();
			_uplevelEffect = null;
		}
		...
		private function bagItemUpdateHandler(e:BagInfoUpdateEvent):void
		{
			...
			for(var i:int = 0;i < tmpItemIdList.length; i++)
			{
				furnaceInfo.updateBagToFurnace(tmpItemIdList[i],qualityItemListChecker,EQUIPUPLEVEL_MATERIAL_CATEGORYID_LIST);
			}
		}
		...
		{
			if(argItemInfo && PromotionItemTemplateList.getPromotionItemInfo(argItemInfo.templateId))
			{
				return true;
			}
			return false;
...
//			_stoneCell.info = null;
			if(_countLabel)_countLabel.text = "";
			if(_upLevelBtn)_upLevelBtn.enabled = false;
//			_currentSuccessRate = 0;
		...
		private function transformBtnHandler(e:MouseEvent):void
		{
			...
			if(CategoryType.isPetEquip(_resultCell.itemInfo.template.categoryId))
			{
				var pet:PetItemInfo = GlobalData.petList.getFightPet();
				if(pet)
				{
					if(pet.level < _resultCell.itemInfo.template.needLevel)
					{
						QuickTips.show('出战宠物等级不够，无法升级。');
						return;
					}
				}
				else
				{
					QuickTips.show('您没有出战宠物，无法升级宠物装备。');
					return;
				}
			}
			if(_resultCell.itemInfo.template.needLevel > GlobalData.selfPlayer.level)
			{
				QuickTips.show(LanguageManager.getWord("ssztl.furnace.notEnoughLevel"));
				return;
			}
			var tmp:PromotionItemInfo = PromotionItemTemplateList.getPromotionItemInfo(_cells[0].furnaceItemInfo.bagItemInfo.templateId);
			if(!_cells[0].furnaceItemInfo.bagItemInfo.isBind && checkMaterialBind(tmp.materialList))
			{
				MAlert.show(LanguageManager.getWord("ssztl.furnace.hasBindUpgradeStone"),LanguageManager.getWord("ssztl.common.alertTitle"),MAlert.OK | MAlert.CANCEL,null,closeHandler);
			}
			else
			{
				send();
			...
			function send():void
			{
				_mediator.sendUpLevel(_cells[0].furnaceItemInfo.bagItemInfo.place,isPetEquip);
//				clearCells();
				_upLevelBtn.enabled = false;
			}
		}
		...
		override protected function doAlert(argNullTipsVector:Array,argUnNullTipsVector:Array):void
		{
			...
			switch(argTipsType)
			{
				...
				case FurnaceTipsType.ENCHASEFULL:
					...
					QuickTips.show(LanguageManager.getWord("ssztl.furnace.putShenYouOnly"));
					break;
				case FurnaceTipsType.STONE1:
					//					QuickTips.show("只能放入紫色装备！");
					QuickTips.show(LanguageManager.getWord("ssztl.furnace.putOrangeEquipOnly"));
					break;
			}
		}
		...
		override protected function getBackgroundName():Array
		{
			...
			return [LanguageManager.getWord("ssztl.furnace.equip"),
				...
				LanguageManager.getWord("ssztl.common.material"),
				LanguageManager.getWord("ssztl.furnace.upLevelSymbol"),""];
		}		
		...
		{
			//是否优先使用绑定材料
			var tmp:PromotionItemInfo = PromotionItemTemplateList.getPromotionItemInfo(info.templateId);
			if (tmp == null)
				return;
...
//			if(_cells[0].furnaceItemInfo)return FurnaceTipsType.NOSTONE;
		...
		private function lronChecker1(info:ItemInfo):int
		{
			...
			if(CategoryType.isEquip(info.template.categoryId))return FurnaceTipsType.EQUIP;
			if(info.template.categoryId == CategoryType.UPGRADE || info.template.categoryId == CategoryType.UPGRADE_UPLEVEL)return FurnaceTipsType.SUCCESS;
...
//			updateSuccessRate(argEquipItemInfo);
			var tmp:PromotionItemInfo = PromotionItemTemplateList.getPromotionItemInfo(argEquipItemInfo.templateId);
			if(tmp)
			{
				/**费用：系数**/
				_currentMoney =  tmp.copper;
				_useMoneyTextField.text =_currentMoney.toString();
				//			_stoneCell.info = ItemTemplateList.getTemplate(tmp.blackStoneTempId); //需要更新所需材料
				updateShowCell(ItemTemplateList.getTemplate(tmp.targetOrgTempId));
				updateCount();				
			}			
		}
		...
		{
			if(_cells[0].furnaceItemInfo == null)return;
			var tmp:PromotionItemInfo = PromotionItemTemplateList.getPromotionItemInfo(_cells[0].furnaceItemInfo.bagItemInfo.templateId);
			if(tmp == null)return;
			var argTargetTemplateInfo:ItemTemplateInfo = ItemTemplateList.getTemplate(tmp.targetOrgTempId);
			var canUP:Boolean = true;
//			if(argTargetTemplateInfo.needLevel > GlobalData.selfPlayer.level)
//				canUP = false;			
			...
			for(var i:int = 0; i < materialList.length; i++)
			{
				...
				if (itemNum < num)
				{
					...
					_lableArray[i].htmlText = itemNum.toString() + "/" + num;
					
			}			
			if(canUP)
				_upLevelBtn.enabled = true;	
		}
		...
		{
			/**成功率**/
//			_currentSuccessRate = PromotionItemTemplateList.getPromotionItemInfo(argEquipItemInfo.templateId).successRate;
		}
